...
`""""""`'''
    ...
    def frame2_decode(self,d_f2):
        ...
        if not myfile:
            ...
            text_area.grid()
            l3 = Label(d_f3, text='ASCII Values:')
            l3.config(font=('courier', 18))
            l3.grid(pady=10)
            ascii_area = Text(d_f3, width=50, height=10)
            ascii_values = " ".join(str(ord(char)) for char in hidden_data)
            ascii_area.insert(INSERT, ascii_values)
            ascii_area.configure(state='disabled')
            ascii_area.grid()
